...
class UserService {
    ...
    def signup(String login) {
        if (!User.findByLogin(login)) {
            def user = new User(
                login: login,
                ready: new Preferences(happy:1,sad:1,optimistic:1,hopeful:1,fearful:1,amused:1,unamused:1,excited:1,total:8).save(),
                ...
                end: new Preferences(happy:1,sad:1,optimistic:1,pessimistic:1,hopeful:1,fearful:1,angry:1,nostalgic:1,total:8).save(),
